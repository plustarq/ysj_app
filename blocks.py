...
__all__ = ['CBAMLayer','CBAMLayer_gai', 'SPPLayer']
...
'''
    ...
    arXiv:1807.06521
'''
class CBAMLayer_gai(nn.Module):
    def __init__(self, channel, reduction=16, spatial_kernel=7):
        super(CBAMLayer_gai, self).__init__()
        # channel attention, //'地板除' 3//2=1
    ...
    def forward(self, x):
        ...
        channel_out = self.sigmoid(max_out + avg_out)#+ avg_out
        x1 = channel_out * x

        # spatial attention
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        avg_out = torch.mean(x, dim=1, keepdim=True)
        spatial_out = self.sigmoid(self.conv(torch.cat([max_out, avg_out], dim=1)))#avg_out
        x2 = spatial_out * x
        return x1+x2#channel_out*spatial_out*x
